...
summarizeReadCounts = function(experiment, scale=1e+7, chrom=c(1:22,'X','Y'), output=NULL) {
    bam = scanBam(BamFile(experiment), param=ScanBamParam(what=c('rname','pos','qwidth')))[[1]]
...
summarizePeakSignals = function(experiment, suffix=NULL, chrom=c(1:22,'X','Y'), output=NULL) {
    peak = lapply(experiment, function(x) fread(x, select=c(1,2,3,7,8),
    ...
    if (length(experiment) > 1) {
        if (length(suffix) < length(experiment)) {
            print('Invalid suffix.')
            suffix = 1:length(experiment)
        }
